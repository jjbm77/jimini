...
Definir los requisitos de ingesta durable de mensajes de Telegram, incluyendo el buffer persistente, el protocolo de lease (claim atómico, backoff exponencial, dead letter queue), el reclaim de stale locks vía pg_cron, y el esquema multi-media del buffer. Es la base de todas las demás capacidades de ingesta del sistema.
...
El webhook SHALL detectar comandos de Telegram (mensajes que comienzan con `/`) antes de insertar en el buffer. Los comandos conocidos SHALL ser manejados específicamente: `/recurrencias`, `/tareas`, `/vacaciones`, y `/finde` SHALL ser respondidos directamente sin pasar por el buffer. Los comandos `/recurrencia <texto>` (crear recurrencia) y `/vacaciones <fecha>` (crear modo vacaciones) SHALL tener handlers específicos.
...
- **THEN** consulta tareas con `fecha_vence BETWEEN CURRENT_DATE AND CURRENT_DATE + 7` + vencidas
- **AND** responde con la lista agrupada por día
- **AND** no inserta nada en el buffer

#### Scenario: Comando /mes
- **WHEN** el webhook recibe `/mes` o `/mes N`
- **THEN** calcula el grid del mes correspondiente
- **AND** responde con el calendario ASCII
- **AND** no inserta nada en el buffer
