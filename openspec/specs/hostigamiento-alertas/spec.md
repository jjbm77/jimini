...
Definir los requisitos del motor de hostigamiento de Pepe Grillo: el worker de evaluación continua, el cálculo de nivel por días de vencimiento, la frecuencia de repetición por nivel, el horario activo, los botones de snooze y resolución, y los mensajes de alerta formateados por nivel. Es el alma del sistema — la diferencia entre un gestor de tareas pasivo y un asistente que fuerza al usuario a responder.
...
- **THEN** el sistema ejecuta `UPDATE tareas SET proxima_alerta_bloqueada_hasta = TOMORROW 09:00` WHERE id = <tarea>
- **AND** `fecha_vence` permanece sin cambios
...
Todas las alertas SHALL incluir un botón `[✅ Completar]`. Las alertas de niveles 3 y 4 SHALL además incluir un botón `[🗑️ Descartar]`. Al presionar [Completar], el sistema SHALL setear `estado='completado'`. Al presionar [Descartar], el sistema SHALL setear `estado='descartado'`.
